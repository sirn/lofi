...
impl TerminalScreen {
    ...
    /// transcript log, so this identifies the cursor's row.
    fn tinted_row_text(&self) -> Option<String> {
        self.cells.iter().find_map(|row| {
...
impl Tui {
    ...
    pub fn wait_for_tinted(&mut self, needle: &str, timeout: Duration) {
        let start = Instant::now();
        while start.elapsed() < timeout {
            if let Some(text) = self.tinted_row_text() {
        ...
        panic!(
            "timed out waiting for tinted row containing {needle:?}; rendered screen:\n{screen}"
        );
    }
