...
fn policy_modes_and_yolo_have_process_level_wire_behavior() {
    let server = MockServer::start(vec![
        tool_response(
            "readonly-call",
            r#"return await lofi.bash({ cmd: "printf readonly-ran > readonly.txt" });"#,
        ),
    ...
    assert!(!fixture.workspace.join("readonly.txt").exists());
    let requests = server.requests();
...
fn filesystem_secret_boundaries_hold_across_symlinks_and_chunked_output() {
    let server = MockServer::start(vec![
        tool_response(
            "boundary-call",
            r#"const outside = await lofi.read("../config.toml");
const linked = await lofi.read("linked-secret.txt");
const huge = await lofi.bash({ cmd: "printf $FIXTURE_SECRET; i=0; while [ $i -lt 1200 ]; do printf '-chunk-%03d' $i; i=$((i+1)); done; printf $FIXTURE_SECRET" });
return { outside, linked, huge }; "#,
        ),
    ...
    let requests = server.requests();
    assert_eq!(requests.len(), 2);
    assert!(requests[1].body.contains("path escapes workspace"));
    assert!(requests[1].body.contains("[redacted]"));
    assert!(!requests[1].body.contains("fixture-secret-value"));
    assert!(!fixture
        .events()
