...
fn new_session_and_explicit_resume_select_the_requested_history() {
    ...
    let first_id = first_path.file_stem().unwrap().to_str().unwrap().to_string();
    ...
    let mut resumed = fixture.spawn(&["--resume", &first_id[..8]]);
...
fn hard_context_pressure_compacts_and_silently_continues_the_tool_cycle() {
    ...
    let current = current.replace("context_window = 100000", "context_window = 100", 1);
    std::fs::write(
        &fixture.config,
