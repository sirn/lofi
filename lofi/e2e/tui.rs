...
fn reloaded_native_tool_details_show_their_caret_before_any_expansion() {
    let server = MockServer::start(vec![
        tool_response(
            ...
            r#"return await lofi.read("reloaded-hidden-detail.txt");"#,
        ),
    ...
    let read_row = reloaded
        .screen_row("Tool read")
        .unwrap_or_else(|| panic!("missing Tool read row; screen:\n{}", reloaded.screen_text()));
    assert!(
        read_row.contains('▸'),
        "reloaded hidden native detail has no collapsed caret before expansion: {read_row}"
    );
}
