...
fn wait_tinted_row(tui: &Tui, needle: &str) {
    ...
    while start.elapsed() < WAIT {
        if tui.tinted_row_text().as_deref().is_some_and(|row| row.contains(needle)) {
            return;
        }
...
return "mini-done";
    ...
    select_transcript_row(&mut tui, "Exec");
    tui.send(b"\r");
    tui.wait_for_screen("mini-exec-00", WAIT);
    wait_tinted_row(&tui, "mini-exec-00");
    assert!(!tui.screen_text().contains("mini-exec-10"), "screen:\n{}", tui.screen_text());
    ...
    assert!(!tui.screen_text().contains("mini-exec-00"), "screen:\n{}", tui.screen_text());
    tui.send(b"\x1b[1;2B");
    std::thread::sleep(std::time::Duration::from_millis(50));
    assert!(
        tui.tinted_row_text()
    ...
    assert!(tui.screen_row("Exec").is_some_and(|row| row.contains("▸")));
}

#[test]
fn reloaded_details_show_their_carets_before_any_expansion() {
    let server = MockServer::start(vec![
        tool_response(
