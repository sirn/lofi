...
async fn list_models(cli: &Cli) -> anyhow::Result<()> {
    ...
    let mut config = lofi_core::config_loader::load_config(&config_path)
        .await
    ...
    if let (Some(name), Some(key)) = (&cli.provider, &cli.api_key) {
        lofi_core::config_loader::apply_api_key_override(&mut config, name, key.clone())
