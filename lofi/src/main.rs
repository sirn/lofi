...
//!
//! A thin clap CLI that dispatches into `lofi_core`:
//! - `--list-models` prints `provider/id — name` lines and exits;
//! - `--print <prompt>` runs one non-interactive turn via [`lofi_core::run_print`];
//! - otherwise the interactive TUI is launched via [`lofi_core::run_interactive`].
//!
...
use anyhow::Context;
use clap::Parser;
use lofi_core::{InteractiveOptions, ModelRegistry, PrintOptions};
...
async fn main() -> anyhow::Result<()> {
    ...
    if cli.list_models {
        ...
        lofi_core::run_print(opts).await?;
    } else {
        let opts = build_interactive_opts(&cli, root);
        lofi_core::run_interactive(opts).await?;
    }
    Ok(())
}
...
async fn list_models(cli: &Cli) -> anyhow::Result<()> {
    ...
    // Resolve the selected provider with the same rules as the agent run
    // (explicit > default_provider > single provider), so `--api-key` applies
    // to the provider the user actually targets instead of being silently
    // dropped when `--provider` is omitted.
