...
mod tests {
    ...
    #[test]
    fn auto_compact_threshold_reserved_default() {
        let cfg = AutoCompactConfig::default();
        // 200k window, 20k reserved -> 180k.
        assert_eq!(cfg.threshold(200_000), Some(180_000));
        // saturating sub when reserved >= window -> no positive threshold.
        assert_eq!(cfg.threshold(10_000), None);
        assert_eq!(cfg.threshold(0), None);
    }

    #[test]
    fn auto_compact_threshold_caps_lower_only() {
        // Absolute cap lowers the threshold; a cap above the reserve-based
        // threshold is inert.
        let with_cap = AutoCompactConfig {
            max_context_tokens: Some(150_000),
            ..AutoCompactConfig::default()
        };
        assert_eq!(with_cap.threshold(1_000_000), Some(150_000));
        assert_eq!(with_cap.threshold(200_000), Some(150_000)); // min(180k, 150k)
        // A cap above the reserve-based threshold is inert.
        let inert_cap = AutoCompactConfig {
            max_context_tokens: Some(190_000),
            ..AutoCompactConfig::default()
        };
        assert_eq!(inert_cap.threshold(200_000), Some(180_000));
        // Ratio cap lowers; out-of-range ratio is ignored.
        let with_ratio = AutoCompactConfig {
            context_ratio: Some(0.5),
            ..AutoCompactConfig::default()
        };
        assert_eq!(with_ratio.threshold(200_000), Some(100_000));
        let bad_ratio = AutoCompactConfig {
            context_ratio: Some(1.5),
            ..AutoCompactConfig::default()
        };
        assert_eq!(bad_ratio.threshold(200_000), Some(180_000));
    }

    #[test]
    fn auto_compact_config_serde_defaults() {
        // Omitted fields take defaults: enable=true, reserved=20k, caps None.
        let json = "{}";
        let cfg: AutoCompactConfig = serde_json::from_str(json).unwrap();
        assert!(cfg.enable);
