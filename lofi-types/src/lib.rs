...
pub struct Model {
    ...
    /// carries a path (e.g. a plexus proxy routing one base URL to several
...
/// requests. This lets one endpoint (e.g. a plexus proxy) route to several
...
/// `id` collision). lofi has no pi-ai model catalog, so discovered models
...
pub struct Config {
    ...
    /// resolved key (or `no_auth`) supplies the default model when `--model`
    /// is omitted.
