...
impl Api {
    ...
    /// Default API base URL for this protocol, used when a provider entry omits
    /// `base_url`. Kept here so the config loader can fill it in without a
    /// hardcoded table elsewhere.
    #[must_use]
...
/// Per-model API override for auto-discovered models: maps a remote api-type
/// string reported by the `/v1/models` endpoint to the lofi [`Api`] and an
/// optional relative path appended to the provider's `base_url` when sending
/// requests. This lets one endpoint (e.g. an OpenAI-compatible proxy) route to several
/// upstream APIs behind a single provider.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct ApiTypeMapping {
    pub api: Api,
    /// Relative path appended to the provider `base_url` for this API.
    #[serde(default)]
...
pub struct AutoModelsConfig {
    ...
    /// Map of remote api-type string to [`ApiTypeMapping`].
    #[serde(default)]
    pub api_type_mappings: IndexMap<String, ApiTypeMapping>,
    /// Key into `api_type_mappings` used when a model reports no api-type.
    #[serde(default)]
    ...
    /// How the remote API reports pricing values. `per_token` (default)
    /// multiplies each value by 1M to get a per-1M-token cost; `per_million`
    /// uses values as-is.
    #[serde(default)]
    pub pricing_convention: PricingConvention,
    /// Dot-notation paths to the per-model pricing fields in each entry of
    /// the `/v1/models` response. When unset, defaults to `pricing.prompt`,
    /// `pricing.completion`, `pricing.input_cache_read`, and
    /// `pricing.input_cache_write` (the OpenRouter/Plexus convention).
    #[serde(default)]
...
///
/// Mirrors the Pi `hm-remote-models` extension's `pricingFieldMappings` so a
/// proxy whose pricing lives under non-standard keys can be configured without
/// code changes. An empty path means no remote source for that cost dimension.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
pub struct ProviderConfig {
    ...
    pub api_type: Api,
    /// Base URL for the provider's API. When omitted, defaults to
    /// [`Api::default_base_url`] for the provider's `api_type`.
    #[serde(default)]
    pub base_url: Option<String>,
...
mod tests {
    ...
    fn config_round_trips() {
        ...
        providers.insert(
            "openai".to_string(),
            ProviderConfig {
                api_type: Api::OpenAiCompletions,
