...
pub struct ProviderConfig {
    ...
    /// auth headers regardless of `api_key`/`headers`. Without this, a
    /// provider with no key and no credential-bearing header is not
    /// available — so the zero-config `OpenAI` default still requires
    /// `OPENAI_API_KEY`.
    #[serde(default)]
    pub no_auth: bool,
}
