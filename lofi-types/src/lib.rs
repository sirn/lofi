...
pub enum ContentBlock {
    ...
    /// Opaque metadata for the preceding provider part. Google Gemini uses
    /// this to retain a thought signature on text and function-call parts.
    /// Keeping it adjacent lets the Google IR restore the signature to the
    /// exact part without adding provider fields to neutral content blocks.
    PartSignature {
        provider: String,
        model: String,
        signature: String,
...
pub enum StreamingEvent {
    ...
    /// Opaque signature attached to the preceding streamed provider part.
    PartSignature {
        provider: String,
        model: String,
        signature: String,
