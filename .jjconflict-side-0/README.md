...
| Diagnostics and resource lifecycle | **Partial** | Debug toggles, debug line shape, transcript result release, large tree-picker memory release, and large transcript resume memory release are exercised. | Add debug counters, long-stream memory bounds, repeated session and model switches, dropped consumers, temporary log cleanup, graceful live-run shutdown, and leak checks outside Linux/glibc. |
...
| TUI slash commands and modals | **Partial** | Autocomplete, help, session, theme, model, thinking, service, jobs, debug, recall, clear, compact, new, resume, tree, quit, exit, unknown commands, and topmost input priority when a policy confirmation overlaps the tree picker. | Add full keyboard navigation, scrolling, copy, cancellation, and narrow-terminal tests for each modal. |
