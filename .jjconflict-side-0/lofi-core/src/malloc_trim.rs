...
/// Calling `release_freed_memory` only at the final `Ok` misses
/// flush-error and early-emit returns.
#[must_use = "dropping immediately trims before the burst"]
pub(crate) struct ReleaseFreedMemoryOnDrop;
