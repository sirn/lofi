...
/// for compatible providers that return that shape instead.
fn reasoning_item_text(item: &Value) -> Option<String> {
    ["summary", "content"].into_iter().find_map(|field| {
        let text = item
            ...
            .filter_map(trim_reasoning_summary_part)
            .collect::<Vec<_>>()
            .join("\n\n");
...
/// Trim one structured summary part and discard `OpenAI`'s empty placeholder.
/// The comment is removed only when it is the whole body (optionally after a
/// bold status header), so literal comments embedded in real content survive.
fn trim_reasoning_summary_part(part: &str) -> Option<&str> {
    let part = part.trim();
    if part.is_empty() {
        return None;
    }
...
mod tests {
    ...
    #[test]
    fn completed_reasoning_item_drops_empty_placeholder_parts() {
        let ev = json!({
            "type":"response.output_item.done",
            "item":{
                "type":"reasoning",
