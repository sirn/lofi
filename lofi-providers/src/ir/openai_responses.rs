        ...
        "response.output_item.done" => {
            if let Some(item) = v.get("item") {
                match item.get("type").and_then(Value::as_str) {
                    ...
                    Some("reasoning") => {
                        // Some Responses-compatible providers omit summary
                        // deltas but include the completed reasoning item. Use
                        // that as a fallback without duplicating streamed text.
                        let item_id = item.get("id").and_then(Value::as_str).unwrap_or("");
                        if !state.reasoning_items_with_deltas.contains(item_id) {
                            if let Some(text) = reasoning_item_text(item) {
                                out.push(StreamingEvent::ThinkingDelta(text));
                            }
                        }
                    }
                    _ => {}
