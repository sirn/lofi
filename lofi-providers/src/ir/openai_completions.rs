...
fn map_openai_chat_event(v: &Value, state: &mut ChatMapperState) -> Result<Vec<StreamingEvent>> {
    ...
    if let Some(arr) = delta.get("tool_calls").and_then(Value::as_array) {
        for tc in arr {
            ...
            let signature = extra_content_signature(tc)
                .map(|(namespace, signature)| (namespace.to_string(), signature.to_string()));
            if let (Some(idx), Some(signature)) = (idx, signature.as_ref()) {
                if id.is_none() && !state.index_to_id.contains_key(&idx) {
                    state
                        .pending_signature_by_index
                        .insert(idx, signature.clone());
                }
            }
            let signature = signature
                .or_else(|| idx.and_then(|idx| state.pending_signature_by_index.remove(&idx)));
            if let Some((namespace, signature)) = signature {
                let target = idx
