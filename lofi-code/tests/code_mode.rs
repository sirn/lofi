...
async fn job_periodic_tick_goes_to_ui_not_model() {
    ...
    // Model-injection queue must be empty: a running job stays async.
    assert!(jobs.drain_notices().is_empty(), "tick leaked to model");
    let ui = jobs.drain_ui_notices();
    assert!(!ui.is_empty(), "no tick rendered to UI");
    assert!(ui[0].contains("running"), "tick: {}", ui[0]);
}

#[tokio::test]
async fn job_terminal_goes_to_both_queues() {
    let dir = tempfile::tempdir().unwrap();
    let jobs = lofi_code::tools::JobRegistry::new();
    let mut cx = trusted_ctx(dir.path());
    cx.jobs = jobs.clone();
    ...
    exec(src, &cx, &ExecOptions::default()).await.unwrap();
    let term = jobs.drain_notices();
    assert_eq!(term.len(), 1, "terminal notices: {term:?}");
    let ui = jobs.drain_ui_notices();
    assert!(!ui.is_empty(), "no UI notices: {ui:?}");
}
#[tokio::test]
