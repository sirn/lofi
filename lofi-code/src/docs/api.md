...
## lofi.job_spawn({ cmd, timeoutMs?, notify?, notifyIntervalMs? })
...
- `notifyIntervalMs` (number, optional) — turn on periodic progress pings
  ...
  `notify: true`; a separate `job_notify` call is not needed.
...
## lofi.job_notify({ id, enabled?, intervalMs?, changed? })
