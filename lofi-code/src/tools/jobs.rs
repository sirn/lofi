...
impl JobRegistry {
    ...
    /// notices are discarded. Existing subscribers stay attached so the
    /// same registry can back the next session without rebuilding the agent.
    /// Hosts must also replace their receiver to discard notices that were
    /// already delivered to its channel before this reset.
    pub fn reset(&self) {
