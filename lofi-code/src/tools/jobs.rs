...
async fn run_job(
    // The registry alone is sufficient to publish notices. Taking the whole
    // BuiltinTools would leak its tool callback (an UnboundedSender per
    // execute_tools round) into the spawned task's lifetime, keeping the
    // engine-to-UI channel — and therefore the run — alive until the job
    // exits.
    jobs: JobRegistry,
