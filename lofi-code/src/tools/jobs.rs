...
struct Inner {
    ...
    /// Notice lines (periodic ticks and terminal transitions) awaiting pickup
    /// by the host UI, which turns them into ordinary queued prompts.
...
impl JobRegistry {
    ...
    /// Take all queued notices, leaving the queue empty. The host UI drains
    /// this on its tick and turns the batch into a queued prompt.
    #[must_use]
...
/// Queue a notice for the host UI to pick up on its next tick.
fn push_notice(tools: &BuiltinTools, text: String) {
    tools
        .jobs
        .inner
        .notices
