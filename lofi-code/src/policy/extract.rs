...
pub fn build_wrapper_map(
    entries: &[lofi_types::WrapperRuleConfig],
) -> WrapperRuleMap {
    entries
        .iter()
...
pub fn extract_commands(
    ...
    for seg in &segments {
        ...
        if word_tokens.is_empty() { continue; }

        // Skip compound-command keywords
        if SKIP_SEGMENT.contains(&word_tokens[0]) { continue; }
        ...
        if word_slice.is_empty() { continue; }

        let cmd_name = word_slice[0].to_string();
        if cmd_name.is_empty() { continue; }

        let words: Vec<String> = word_slice.iter().map(std::string::ToString::to_string).collect();
        ...
        if let Some(kind) = wrappers.get(&word_slice[0].to_ascii_lowercase()) {
            if let Some(inner_words) = unwrap_wrapper(*kind, &word_slice.iter().map(std::string::ToString::to_string).collect::<Vec<_>>()) {
                if !inner_words.is_empty() {
                    let inner_text = inner_words.join(" ");
                    if let Ok(inner_tokens) = tokenize(&inner_text) {
                        results.extend(extract_commands(&inner_tokens, CommandSource::WrapperArg, wrappers));
                    }
                }
            }
        }
...
fn unwrap_wrapper(kind: WrapperKind, words: &[String]) -> Option<Vec<String>> {
    match kind {
        ...
        WrapperKind::UtilityOperand | WrapperKind::Xargs => {
            ...
            for w in &words[1..] {
                if !saw_dash && w == "--" { saw_dash = true; continue; }
                if !saw_dash && w.starts_with('-') { continue; }
                return Some(words.iter().skip_while(|x| !std::ptr::eq(*x, w)).cloned().collect());
            }
            None
        }
        WrapperKind::Env => {
            ...
            while i < words.len() {
                let w = &words[i];
                if !saw_dash && w == "--" { saw_dash = true; i += 1; continue; }
                if !saw_dash && w.starts_with('-') { i += 1; continue; }
                if is_assignment(w) { i += 1; continue; }
        ...
        WrapperKind::DockerRun => {
            ...
            if !matches!(sub.as_str(), "run" | "exec" | "create") { return None; }
            let mut i = 2;
            while i < words.len() {
                let val = &words[i];
                if val == "--" { i += 2; return words.get(i..).map(<[std::string::String]>::to_vec); }
                if val.starts_with("--") && val.contains('=') { i += 1; continue; }
                if is_docker_bool_flag(val) { i += 1; continue; }
                if val.starts_with('-') && !val.starts_with("--") && val.len() > 2 { i += 1; continue; }
                if val.starts_with("--") { i += 2; continue; }
                if val.starts_with('-') && val.len() == 2 { i += 2; continue; }
...
const DOCKER_BOOL_FLAGS: &[&str] = &[
    "--detach", "--interactive", "--tty", "--rm", "--privileged", "--init",
    "--read-only", "--publish-all", "--oom-kill-disable", "--no-healthcheck",
    "--sig-proxy", "--help", "-d", "-i", "-t", "-P",
];
