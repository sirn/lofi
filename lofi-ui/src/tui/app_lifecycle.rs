...
impl App {
    ...
    /// first frame. Arms `pending_prompt_start`; `apply_event` dedups the
    /// matching `TurnStart` against this turn (the flag prevents false
    /// matches during transcript replay, where consecutive identical
    /// prompts are legal).
