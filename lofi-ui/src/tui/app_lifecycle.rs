...
impl App {
    pub(super) fn new(
        ...
        Self {
            ...
            collapsed_turns: RefCell::new(CollapsedTurnCache::new()),
    ...
    pub(super) fn materialize_turn(&self, idx: usize) -> Arc<Turn> {
        ...
        if !self.verbose {
            if let Some(turn) = self.collapsed_turns.borrow().get(idx) {
                return turn;
            }
        }
        let prompt = self
        ...
        if !self.verbose {
            self.collapsed_turns.borrow_mut().insert(idx, turn.clone());
        }
        turn
    }
