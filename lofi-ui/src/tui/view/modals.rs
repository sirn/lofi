...
pub(super) fn render_confirm_modal(f: &mut Frame, area: Rect, app: &App) {
    ...
    let content_w = w.saturating_sub(4).max(1) as usize;
    let cmd_lines = prim::wrap(&req.command, content_w);
    // Header, intro, label, command panel, buttons, and help use seven fixed
    // inner rows around the naturally wrapped command body.
