...
fn detail_row_count(lines: &[String], width: usize, deco: &[Span<'static>]) -> usize {
    let inner = width.saturating_sub(prim::span_width(deco) + 3);
...
fn detail_box(
    ...
    let start = expanded
        ...
        .min(max);
    let visible = &rows[start..total.min(start + DETAIL_VIEW_ROWS)];
    ...
    for row in 0..DETAIL_VIEW_ROWS {
        let shown = visible.get(row).map_or("", String::as_str);
        let used = prim::width(shown);
        let thumb_row = if max == 0 {
            None
        } else {
            Some(start * (DETAIL_VIEW_ROWS - 1) / max)
        };
        out.push(target(prim::rline(
            deco.clone(),
            vec![
                ...
                Span::styled(if thumb_row == Some(row) { "┃" } else { "│" }, border),
            ],
        )));
    }
    out.push(target(prim::rline(
...
impl ExecBlock<'_> {
    fn render_window(&self, cx: &Cx, range: std::ops::Range<usize>) -> RenderWindow {
        ...
        if self.tool.done {
            out.extend(exec_result_lines(self.tool, t, w));
        }
...
fn exec_result_lines(tool: &ToolCall, t: Theme, w: usize) -> Vec<RenderLine> {
    let mut out = Vec::new();
    ...
    // Successful exec results repeat the native-tool rows above. Keep only
    // failure output here so an error is never swallowed.
    if !tool.is_error {
        return out;
    }
