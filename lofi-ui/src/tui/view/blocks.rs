...
mod tests {
    ...
    fn dbg3() {
        ...
        for b in &analyze(src, Theme::default(), Style::default()) {
            let (k, r) = match b {
                MdBlock::Blank { src } => ("blank", src.clone()),
                MdBlock::Paragraph { src, .. } => ("para", src.clone()),
                MdBlock::Heading { src, .. } => ("heading", src.clone()),
                MdBlock::Code { fence, .. } => ("code", fence.clone()),
                MdBlock::Table { src, .. } => ("table", src.clone()),
                MdBlock::Quote { src, .. } => ("quote", src.clone()),
            };
            eprintln!("{} {:?}", k, r);
        }
        panic!("x");
