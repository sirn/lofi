...
impl Component for Thinking<'_> {
    fn lines(&self, cx: &Cx) -> Vec<RenderLine> {
        ...
        let key = if working {
            DetailKey::Thinking(self.block.id)
        } else {
            use std::hash::{Hash, Hasher};
            let mut hasher = std::collections::hash_map::DefaultHasher::new();
            text.hash(&mut hasher);
            self.block.elapsed.hash(&mut hasher);
            DetailKey::Thinking(hasher.finish())
        };
        ...
        .with_detail(key.clone(), text.lines().count(), true);
...
impl ExecBlock<'_> {
    fn render_window(&self, cx: &Cx, range: std::ops::Range<usize>) -> RenderWindow {
        ...
        if self.tool.done {
            out.extend(exec_result_lines(self.tool, t, w, false));
        }
