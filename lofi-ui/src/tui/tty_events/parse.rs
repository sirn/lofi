...
// Adapted from crossterm 0.29.0 event/sys/unix/parse.rs.
// CSI ? 997 is a color-scheme event. A finished unknown DEC private
// report is an error so the reader can drop it (crossterm#1104).
...
pub(crate) fn parse_event(buffer: &[u8], input_available: bool) -> io::Result<Option<Parsed>> {
    ...
    match buffer[0] {
        b'\x1B' => {
            if buffer.len() == 1 {
                if input_available {
                    ...
                    Ok(Some(Parsed::Event(Event::Key(KeyCode::Esc.into()))))
                }
            } else {
                match buffer[1] {
                    b'O' => {
                        if buffer.len() == 2 {
                            ...
                            match buffer[2] {
                                b'D' => Ok(Some(Parsed::Event(Event::Key(KeyCode::Left.into())))),
                                b'C' => Ok(Some(Parsed::Event(Event::Key(KeyCode::Right.into())))),
                                b'A' => Ok(Some(Parsed::Event(Event::Key(KeyCode::Up.into())))),
                                b'B' => Ok(Some(Parsed::Event(Event::Key(KeyCode::Down.into())))),
                                b'H' => Ok(Some(Parsed::Event(Event::Key(KeyCode::Home.into())))),
                                b'F' => Ok(Some(Parsed::Event(Event::Key(KeyCode::End.into())))),
                                // F1-F4
                                val @ b'P'..=b'S' => Ok(Some(Parsed::Event(Event::Key(
                    ...
                    _ => parse_event(&buffer[1..], input_available).map(|event_option| {
                        event_option.map(|event| {
                            if let Parsed::Event(Event::Key(key_event)) = event {
                                let mut alt_key_event = key_event;
                                alt_key_event.modifiers |= KeyModifiers::ALT;
                                Parsed::Event(Event::Key(alt_key_event))
                            } else {
                                event
                            }
                        })
        ...
        b'\r' => Ok(Some(Parsed::Event(Event::Key(KeyCode::Enter.into())))),
        // Issue #371: \n = 0xA, which is also the keycode for Ctrl+J. The only reason we get
        // newlines as input is because the terminal converts \r into \n for us. When we
        // enter raw mode, we disable that, so \n no longer has any meaning - it's better to
        // use Ctrl+J. Waiting to handle it here means it gets picked up later
        b'\n' if !crossterm::terminal::is_raw_mode_enabled().unwrap_or(false) => {
            Ok(Some(Parsed::Event(Event::Key(KeyCode::Enter.into()))))
        }
        b'\t' => Ok(Some(Parsed::Event(Event::Key(KeyCode::Tab.into())))),
        b'\x7F' => Ok(Some(Parsed::Event(Event::Key(KeyCode::Backspace.into())))),
        c @ b'\x01'..=b'\x1A' => Ok(Some(Parsed::Event(Event::Key(KeyEvent::new(
            KeyCode::Char((c - 0x1 + b'a') as char),
            KeyModifiers::CONTROL,
        ))))),
        c @ b'\x1C'..=b'\x1F' => Ok(Some(Parsed::Event(Event::Key(KeyEvent::new(
            KeyCode::Char((c - 0x1C + b'4') as char),
            KeyModifiers::CONTROL,
        ))))),
        b'\0' => Ok(Some(Parsed::Event(Event::Key(KeyEvent::new(
            KeyCode::Char(' '),
            KeyModifiers::CONTROL,
        ))))),
...
pub(crate) fn parse_csi(buffer: &[u8]) -> io::Result<Option<Parsed>> {
    ...
    Ok(input_event.map(Parsed::Event))
}
...
pub(crate) fn parse_csi_cursor_position(buffer: &[u8]) -> io::Result<Option<Parsed>> {
    // ESC [ Cy ; Cx R
    ...
    Ok(Some(Parsed::CursorPosition(x, y)))
}

fn parse_csi_color_scheme(buffer: &[u8]) -> io::Result<Option<Parsed>> {
    // CSI ? 997 ; 1 n  dark
    // CSI ? 997 ; 2 n  light
    if buffer.len() < 8 {
        return Ok(None);
...
fn parse_csi_keyboard_enhancement_flags(buffer: &[u8]) -> io::Result<Option<Parsed>> {
    ...
    Ok(Some(Parsed::KeyboardEnhancementFlags(flags)))
}

fn parse_csi_primary_device_attributes(buffer: &[u8]) -> io::Result<Option<Parsed>> {
    ...
    Ok(Some(Parsed::PrimaryDeviceAttributes))
}
...
pub(crate) fn parse_csi_modifier_key_code(buffer: &[u8]) -> io::Result<Option<Parsed>> {
    assert!(buffer.starts_with(b"\x1B[")); // ESC [
                                           //
    let s = std::str::from_utf8(&buffer[2..buffer.len() - 1])
        .map_err(|_| could_not_parse_event_error())?;
    ...
    Ok(Some(Parsed::Event(input_event)))
}
...
pub(crate) fn parse_csi_u_encoded_key_code(buffer: &[u8]) -> io::Result<Option<Parsed>> {
    assert!(buffer.starts_with(b"\x1B[")); // ESC [
    assert!(buffer.ends_with(b"u"));
...
pub(crate) fn parse_csi_special_key_code(buffer: &[u8]) -> io::Result<Option<Parsed>> {
    assert!(buffer.starts_with(b"\x1B[")); // ESC [
...
pub(crate) fn parse_csi_rxvt_mouse(buffer: &[u8]) -> io::Result<Option<Parsed>> {
    // rxvt mouse encoding:
    // ESC [ Cb ; Cx ; Cy ; M
    ...
    Ok(Some(Parsed::Event(Event::Mouse(MouseEvent {
        kind,
        column: cx,
        row: cy,
        modifiers,
...
pub(crate) fn parse_csi_normal_mouse(buffer: &[u8]) -> io::Result<Option<Parsed>> {
...
pub(crate) fn parse_csi_sgr_mouse(buffer: &[u8]) -> io::Result<Option<Parsed>> {
    // ESC [ < Cb ; Cx ; Cy (;) (M or m)
...
pub(crate) fn parse_csi_bracketed_paste(buffer: &[u8]) -> io::Result<Option<Parsed>> {
    ...
    if !buffer.ends_with(b"\x1b[201~") {
        ...
        Ok(Some(Parsed::Event(Event::Paste(paste))))
    }
}
...
mod tests {
    ...
    fn test_esc_key() {
        assert_eq!(
            ...
            Some(Parsed::Event(Event::Key(KeyCode::Esc.into()))),
        );
    }
    ...
    fn test_alt_key() {
        assert_eq!(
            ...
            Some(Parsed::Event(Event::Key(KeyEvent::new(
                KeyCode::Char('c'),
                KeyModifiers::ALT
            )))),
        );
    ...
    fn test_parse_event_subsequent_calls() {
        ...
        assert_eq!(
            ...
            Some(Parsed::CursorPosition(9, 19))
        );

        // parse_csi
        assert_eq!(
            parse_event(b"\x1B[D", false).unwrap(),
            Some(Parsed::Event(Event::Key(KeyCode::Left.into()))),
        );
        ...
        assert_eq!(
            ...
            Some(Parsed::Event(Event::Key(KeyCode::Delete.into()))),
        );

        // parse_csi_bracketed_paste
        assert_eq!(
            parse_event(b"\x1B[200~on and on and on\x1B[201~", false).unwrap(),
            Some(Parsed::Event(Event::Paste("on and on and on".to_string()))),
        );
    ...
    fn test_parse_event() {
        assert_eq!(
            ...
            Some(Parsed::Event(Event::Key(KeyCode::Tab.into()))),
        );
    }
    ...
    fn test_parse_csi_bracketed_paste() {
        ...
        assert_eq!(
            ...
            Some(Parsed::Event(Event::Paste("o\x1B[2D".to_string())))
        );
    }
    ...
    fn test_parse_csi_focus() {
        assert_eq!(
            ...
            Some(Parsed::Event(Event::FocusLost))
        );
    }
    ...
    fn test_parse_csi_u_encoded_key_code_with_types() {
        assert_eq!(
            ...
            Some(Parsed::Event(Event::Key(KeyEvent::new_with_kind(
                KeyCode::Char('a'),
                KeyModifiers::empty(),
                KeyEventKind::Press,
            )))),
