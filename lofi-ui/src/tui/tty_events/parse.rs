...
/// Parse result. Crossterm's private `Parsed` plus color scheme.
...
pub(crate) fn parse_event(
    ...
    match buffer[0] {
                ...
                match buffer[1] {
                    b'O' => {
                        if buffer.len() == 2 {
                            ...
                            match buffer[2] {
                                b'D' => {
                                    Ok(Some(Parsed::Event(Event::Key(KeyCode::Left.into()))))
                                }
                                b'C' => Ok(Some(Parsed::Event(Event::Key(
                                    KeyCode::Right.into(),
                                )))),
                                b'A' => {
                                    Ok(Some(Parsed::Event(Event::Key(KeyCode::Up.into()))))
                                }
                                b'B' => {
                                    Ok(Some(Parsed::Event(Event::Key(KeyCode::Down.into()))))
                                }
                                b'H' => {
                                    Ok(Some(Parsed::Event(Event::Key(KeyCode::Home.into()))))
                                }
                                b'F' => {
                                    Ok(Some(Parsed::Event(Event::Key(KeyCode::End.into()))))
                                }
                                // F1-F4
        ...
        b'\r' => Ok(Some(Parsed::Event(Event::Key(
            KeyCode::Enter.into(),
        )))),
        ...
        b'\n' if !crossterm::terminal::is_raw_mode_enabled().unwrap_or(false) => Ok(Some(Parsed::Event(
            Event::Key(KeyCode::Enter.into()),
        ))),
        b'\t' => Ok(Some(Parsed::Event(Event::Key(KeyCode::Tab.into())))),
        b'\x7F' => Ok(Some(Parsed::Event(Event::Key(
