...
//! an `rquickjs` `AsyncContext` which is `!Send`/`!Sync` (see `lofi_code`).
...
use lofi_core::{Agent, AgentEvent};
        ...
        tokio::select! {
            ...
            maybe_ev = events.next() => {
                match maybe_ev {
                    ...
                    // A read failure surfaces as a terminal error so the loop
                    // does not silently drop input.
                    Some(Err(e)) => {
                        last_err = Some(format!("input read failed: {e}"));
                        break;
                    }
                    // EventStream exhausted (e.g. a closed tty). Treat as
                    // terminal to avoid a tight redraw loop on a perpetually
                    // ready `None`.
                    None => {
