...
/// shared key dispatch — up/down (`↑/↓` or `j`/`k`), Enter to confirm,
/// `Esc`/`q` to cancel — lives in one place ([`App::handle_modal_key`]);
/// each modal implements `confirm` for its own side effects.
trait Modal {
    fn len(&self) -> usize;
    fn selected(&self) -> usize;
    fn set_selected(&mut self, n: usize);
    /// Consume the modal and apply its confirm action to `app`.
    fn confirm(self: Box<Self>, app: &mut App);
}
...
impl Modal for TreePickerState {
    ...
    fn confirm(self: Box<Self>, app: &mut App) {
        app.tree_picker_confirm_inner(*self);
    }
}
...
impl App {
  ...
  Esc          clear input\n  Ctrl+C       Input: cancel run · clear · 2× quit  ·  Nav/Select: back to Input + latest  ·  Ctrl+D  del-char / quit on empty\nNavigate      Tab to enter · j/k or ↑/↓ scroll · h/l or ←/→ move col · 0/^/$ · w/b/e · g/G top/bottom · [ ] jump turns · v select · y yank line · i back\nSelect        move extends selection · y or Enter yank → Input · Tab or Esc back\nCommands\n  /help        this help  ·  /clear  clear log\n  /new         start a fresh session  ·  /resume  pick a past session\n  /tree        roll back to a past turn (edit + resend, or continue)\n  /session     show session info  ·  /verbose  toggle tool detail\n  /quit        exit";
    ...
    /// caller skips normal Input-mode processing).
    fn handle_modal_key(&mut self, k: &KeyEvent) -> bool {
        // Take the active modal out so we can move/confirm it through the
        // trait without aliasing `self`. Re-store it on cancel/move.
        let mut modal: Option<Box<dyn Modal>> = if self.picker.is_some() {
            self.picker.take().map(|m| Box::new(m) as Box<dyn Modal>)
        } else if self.tree_picker.is_some() {
            self.tree_picker.take().map(|m| Box::new(m) as Box<dyn Modal>)
        } else {
            None
        };        let Some(mut m) = modal else { return false };
        match k.code {
            KeyCode::Up | KeyCode::Char('k') => {
                if m.selected() > 0 {
                    m.set_selected(m.selected() - 1);
        ...
        // Not confirmed/cancelled — restore the modal.
        let restored = m;
        // Downcast back by re-storing into the right slot. We track which
        // slot via a probe: confirm already returned, so only move/cancel
        // reaches here, and the modal's concrete type is unchanged.
        self.restore_modal(restored);
        true
    }

    /// Restore a taken modal into its original slot. Called after a move
    /// key; confirm/cancel never reach here.
    fn restore_modal(&mut self, _m: Box<dyn Modal>) {
        // The concrete type is known at the take site, but the trait object
        // erased it. In practice we re-store by checking which field was
        // active — but since `handle_modal_key` already knows which slot it
        // took from, we pass that back. Simplified: we don't downcast; we
        // keep the modal in place instead. See the take-free variant below.
        let _ = _m;
    }
