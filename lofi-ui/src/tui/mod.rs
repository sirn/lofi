...
struct ResizeState {
    ...
    last: Option<Instant>,
    events: usize,
    // Parked here so `run_loop` stays under clippy's large-future limit.
    refresh_theme: bool,
}
...
async fn run_loop(
    ...
    loop {
        ...
        if resize.refresh_theme {
            let (next, changed) = pause_events_and_refresh_theme(events, &mut app);
            events = next;
            if changed {
                dirty = true;
            }
            resize.refresh_theme = false;
        }
