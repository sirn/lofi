...
pub(super) fn handle_select_key(k: &KeyEvent, app: &mut App) {
    ...
    match k.code {
        ...
        KeyCode::Tab | KeyCode::Esc => {
            ...
            if let Some(focus) = &mut app.detail_focus {
                focus.anchor = None;
            } else {
                app.sel = None;
            }
        ...
        MouseEventKind::ScrollUp | MouseEventKind::ScrollDown if in_log => {
            ...
            if let Some(detail) = detail_at(app, m.row) {
                if app.mode == Mode::Navigate {
                    app.detail_focus = Some(DetailFocus {
                        key: detail.key.clone(),
                        cursor: detail.row.unwrap_or(if detail.tail {
                            detail.total.saturating_sub(1)
                        } else {
                            0
                        }),
                        col: cell.1,
            ...
            } else if app.detail_focus.is_some() {
