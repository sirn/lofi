...
/// Blocking terminal input reader that yields [`TuiEvent`]s on a channel.
pub(crate) struct TtyEvents {
    rx: tokio::sync::mpsc::UnboundedReceiver<Result<TuiEvent, io::Error>>,
    restore: ReaderRestore,
}
...
impl Drop for TtyEvents {
    fn drop(&mut self) {
        self.restore.stop.store(true, Ordering::Relaxed);
        let _ = write(&self.restore.stop_w, &[1u8]);
        if let Some(thread) = self.restore.thread.take() {
            let _ = thread.join();
        }
    }
}
