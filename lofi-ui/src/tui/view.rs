...
fn render_log(f: &mut Frame, area: Rect, app: &mut App) {
    ...
    // A committed live turn with an expanded detail may have released its
    // exec results from memory; measure and render it from the durable
    // materialization instead.
    let live_idx = n_turns.saturating_sub(1);
    let phase_started = std::time::Instant::now();
    let last_h = if has_live_turn {
        ...
        if app.needs_durable_materialize(live_idx) {
            let turn = app.materialize_turn(live_idx);
            blocks::render_turn_height(&cx, &turn)
        } else {
            blocks::render_turn_height(&cx, &app.turns[live_idx])
        }
    } else {
        0
    };
        ...
        for i in 0..n_turns {
            ...
            if i < frozen_turns {
                ...
                if turn_start + last_h <= off {
                    ...
                    let phase_started = std::time::Instant::now();
                    let lines = if app.needs_durable_materialize(live_idx) {
                        let turn = app.materialize_turn(live_idx);
                        blocks::render_turn_window(&cx, &turn, start..stop)
                    } else {
                        blocks::render_turn_window(&cx, &app.turns[live_idx], start..stop)
                    };
    ...
    if let Some(focus) = app.detail_focus.as_ref() {
        if let Some(rel) = app.focused_detail_row_index(focus.cursor) {
            app.nav_cursor = off.saturating_add(rel);
            app.nav_col = focus.col;
            if app.mode == Mode::Navigate {
                ...
                // Mark the cursor cell like the transcript cursor so the
                // position is visible, not only the line highlight. The clamp
                // pass above already put focus.col inside this row.
                prim::apply_selection(
                    &mut vis[rel],
                    focus.col,
                    focus.col + 1,
                    app.theme.select_cursor,
                );
            }
        }
    }
    ...
    if (app.detail_focus.is_none() || app.mode == Mode::Select)
        && matches!(app.mode, Mode::Navigate | Mode::Select)
    {
...
fn draw_scrollbar(
    ...
    // The thumb uses the user color while its element owns scroll input.
