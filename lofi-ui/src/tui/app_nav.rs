...
impl App {
    ...
    pub(super) fn first_nonblank_col(&self) -> usize {
        ...
        if let Some(focus) = &self.detail_focus {
            let rel = match self.focused_detail_row_index(focus.cursor) {
                Some(rel) => rel,
                None => return cstart,
            };
    ...
    pub(super) fn nav_word_target(&self, motion: WordMotion) -> usize {
        ...
        let Some((vl, current_col)) = (match &self.detail_focus {
            Some(focus) => self
                .focused_detail_row_index(focus.cursor)
    ...
    pub(super) fn page_up(&mut self) {
        ...
        match self.mode {
            ...
            Mode::Navigate | Mode::Select if self.detail_focus.is_some() => {
                self.nav_move(-(DETAIL_VIEW_ROWS as i32));
            }
