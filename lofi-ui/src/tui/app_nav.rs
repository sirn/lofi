...
impl App {
    ...
    fn invalidate_detail_layout(&mut self, turn: usize) {
        self.collapsed_turns.get_mut().map.remove(&turn);
    ...
    pub(super) fn set_cursor_detail_expanded(&mut self, expanded: bool) -> bool {
        ...
        if expanded {
            let scroll = target
                .tail
                .then(|| target.total.saturating_sub(DETAIL_VIEW_ROWS));
            self.expanded_details
                .insert(target.key, DetailState { turn, scroll });
        } else {
    ...
    pub(super) fn scroll_cursor_detail(&mut self, delta: i32) -> bool {
        ...
        let turn = {
            ...
            let current = state.scroll.unwrap_or_else(|| if target.tail { max } else { 0 });
