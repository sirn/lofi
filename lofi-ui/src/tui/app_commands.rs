...
impl App {
    ...
    pub(super) fn picker_confirm_inner(&mut self, picker: PickerState) {
        ...
        match store::SessionCursor::open(entry.path.clone()).and_then(|cursor| {
    ...
    pub(super) fn open_tree_picker(&mut self) {
        ...
        // the cursor. Labels are still loaded lazily by offset.
        ...
        let entries = build_tree_entries(&snapshot.index, snapshot.leaf_id.as_deref(), path);
        if entries.is_empty() {
    ...
    pub(super) fn tree_picker_confirm_inner(&mut self, picker: &TreePickerState) {
        ...
        let loaded = self
            ...
            .and_then(|cursor| cursor.tree_snapshot())
            .and_then(|snapshot| {
                let lineage = index_lineage(&snapshot.index, &entry.branch_point)?;
                self.rollback_indexed(&path, &lineage, snapshot.file_size)
            });
        if let Err(e) = loaded {
            self.notify(NotifyKind::Error, format!("load session for /tree: {e}"));
            return;
        }
        if let Err(e) = self.branch_from(entry.branch_point) {
