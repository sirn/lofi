...
//! scaled). We ask in cooked mode, before raw mode + alternate screen are
//! engaged, with a short timeout. On any failure (no reply, parse error,
...
fn parse_osc11(buf: &[u8]) -> Option<Rgb> {
    // Locate starting marker ESC ] 1 1 ; r g b :
    let mut i = 0;
    while i + 7 < buf.len() {
        if buf[i] == 0x1b
            && buf[i + 1] == b']'
