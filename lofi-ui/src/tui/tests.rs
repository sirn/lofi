...
#[test]
#[ignore]
fn profile_real_resumed_resize_cache() {
    use ratatui::Terminal;

    let path = std::path::PathBuf::from(
        "/home/sirn/.local/state/lofi/sessions/home-sirn-Dev-src-git.sr.ht-~sirn-lofi/1784908657580_c5b95bfc2ac444df8ab071d0c81eaf4f.jsonl",
    );
    ...
    if a.turns.len() > 1 {
        let n = a.turns.len();
        for turn in &mut a.turns[..n - 1] {
            turn.blocks.clear();
        }
    }
    a.bump_render_epoch();
    for width in [111, 168, 224, 111] {
        let mut terminal = Terminal::new(TestBackend::new(width, 36)).unwrap();
        let started = Instant::now();
        terminal.draw(|frame| view::render(frame, &mut a)).unwrap();
        eprintln!(
            "width={width} elapsed={:?} frozen={:?} cache={} bytes={}",
            started.elapsed(),
            Duration::from_micros(a.render_profile.ensure_frozen_us as u64),
            a.collapsed_turns.borrow().map.len(),
            a.collapsed_turns.borrow().retained_bytes,
        );
    }
}
