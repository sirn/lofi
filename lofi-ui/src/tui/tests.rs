...
#[ignore = "process E2E child; launched by agent_task_panic_is_visible_in_tui_e2e"]
async fn agent_task_panic_tui_e2e_child() {
...
#[test]
fn agent_task_panic_is_visible_in_tui_e2e() {
    ...
    command
        .args([
            "--exact",
            "tui::tests::agent_task_panic_tui_e2e_child",
            "--ignored",
            "--nocapture",
        ])
    ...
    master.flush().unwrap();
    wait_for("agent task panicked: provider settlement panic");
    ...
    let status = loop {
        ...
        if started.elapsed() >= Duration::from_secs(10) {
            ...
            let _ = child.wait();
            panic!(
                "child did not exit; terminal output:\n{}",
                output.lock().unwrap()
            );
        }
        thread::sleep(Duration::from_millis(20));
