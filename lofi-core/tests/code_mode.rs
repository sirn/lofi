...
//! top-level await, the `pi.read`/`pi.write` tool bridge, path-escape errors,
...
async fn pi_write_then_read_round_trip() {
    let dir = tempfile::tempdir().unwrap();
    let src = "await pi.write({ path: 'out/nested/x.txt', text: 'hi' }); return await pi.read('out/nested/x.txt');";
...
async fn uncaught_escape_surfaces_as_sandbox_error() {
    let dir = tempfile::tempdir().unwrap();
    let err = exec(
        "await pi.read('../escape'); return 'ok';",
        &ctx(dir.path()),
        &ExecOptions::default(),
    )
    .await
...
#[tokio::test]
async fn strings_exposed_as_pi_and_pi_strings() {
    let dir = tempfile::tempdir().unwrap();
...
#[tokio::test]
async fn exec_bash_echo_via_pi() {
    let dir = tempfile::tempdir().unwrap();
