...
fn row_labels(cursor: &lofi_core::session::store::SessionCursor) -> Result<Vec<String>> {
    let snap = cursor.tree_snapshot()?;
    Ok(
...
) -> Result<(lofi_core::session::store::SessionCursor, String, String)> {
    let store = SessionStore::new(dir.path().join("sessions"));
    let cursor = store.create_cursor(Path::new("/x"), &"m".into())?;
    let mut trunk = vec![user_ev("p1"), assistant_ev("a1"), turn_end()];
    cursor.append_events(&mut trunk)?;
    let t1 = trunk[2].id.clone();

    let mut a = vec![user_ev("p2"), assistant_ev("a2"), turn_end()];
    cursor.append_events(&mut a)?;
    let a_tip = a[2].id.clone();

    cursor.branch_from(t1)?;
    let mut b = vec![user_ev("pB"), assistant_ev("aB"), turn_end()];
    cursor.append_events(&mut b)?;
    let mut b2 = vec![user_ev("pB2"), assistant_ev("aB2"), turn_end()];
    cursor.append_events(&mut b2)?;
    let b_tip = b2[2].id.clone();
    Ok((cursor, a_tip, b_tip))
}

#[test]
fn tree_lists_branched_off_branch_when_active_is_original() -> Result<()> {
    let dir = tempdir()?;
    let (cursor, a_tip, _b_tip) = two_branch_fixture(&dir)?;
    cursor.branch_from(a_tip)?;
...
#[test]
fn tree_lists_original_branch_when_active_is_branched_off() -> Result<()> {
    let dir = tempdir()?;
    let (cursor, _a_tip, b_tip) = two_branch_fixture(&dir)?;
    cursor.branch_from(b_tip)?;
    let labels = row_labels(&cursor)?;
    for expect in ["user: p1", "user: p2", "user: pB", "user: pB2"] {
        assert!(
            labels.iter().any(|l| l.contains(expect)),
