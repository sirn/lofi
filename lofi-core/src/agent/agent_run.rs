...
/// Appends error `ToolResult` blocks for any trailing assistant `ToolUse`
/// blocks that have no matching result. Called when a turn is abandoned after
/// the assistant message was pushed (cancel, closed channel); the shaped
/// result records the abort rather than a real outcome. Durable replay keeps a
/// user-cancelled partial, and providers like the Anthropic Messages API
/// reject an orphaned `tool_use` (400 "did not find any tool_result blocks"),
/// so every `ToolUse` must have a result before the messages go back out.
