...
impl Agent {
    ...
    pub async fn run(&self, user_prompt: String, tx: Sender<AgentEvent>) -> Result<()> {
        ...
        loop {
            ...
            let finished = match self.run_once_inner(&mut messages, Some(&tx), None, None).await {
                Ok(f) => f,
    ...
    pub async fn run_continuation(
        ...
        loop {
            ...
            match self
                .run_once_inner(&mut *messages, Some(&tx), Some(&mut stats), recall.clone())
                .await
            {
                Ok(true) => {
    ...
    pub async fn run_once(&self, messages: &mut Vec<Message>) -> Result<bool> {
        self.run_once_inner(messages, None, None, None).await
    }
    ...
    async fn run_once_inner(
        ...
        let results = self.execute_tools(&tool_uses, tx, stats, recall.clone()).await?;
