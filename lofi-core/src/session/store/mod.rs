...
/// last event's id (legacy/empty-session fallback), or `None` if the file has
/// no events yet (the root). Active apps pass their logical leaf explicitly.
/// `parent_hint` is how a branch
