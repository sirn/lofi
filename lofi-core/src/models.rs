...
//! [`Provider::list_models`](lofi_providers::Provider::list_models) hits a
...
impl ModelRegistry {
    ...
    pub async fn load_async(config: &Config) -> Result<Self> {
        ...
        // Load the cache once. A successful refresh replaces that provider's
        // cached entries (including with an empty set, so a provider that
        // removed a model isn't resurrected by a later failure); a failed
        // refresh retains the provider's existing cached entries as a fallback.
        let mut cached = read_discovery_cache(&cache_path).unwrap_or_default();
        ...
        for (name, pcfg) in &config.providers {
            ...
            match discover_models(name, pcfg).await {
                Ok(models) => {
                    any_refreshed = true;
                    cached.retain(|m| m.provider != *name);
                    cached.extend(models.iter().cloned());
                    discovered.extend(models);
                }
                Err(remote_err) => {
                    // Fall back to whatever the cache holds for this provider.
                    let provider_cached: Vec<Model> = cached
                        .iter()
                    ...
                    if provider_cached.is_empty() {
                        ...
                        discovered.extend(provider_cached);
                    }
                }
            }
        }
        ...
        if any_refreshed {
            write_discovery_cache(&cache_path, &cached)?;
        }
