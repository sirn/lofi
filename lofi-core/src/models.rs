    ...
    pub fn available(&self) -> Vec<Model> {
        self.models
            ...
            .filter(|m| match self.providers.get(&m.provider) {
                ...
                // or a custom header carrying credentials (header-only auth
                // such as a proxy token). A fully configured provider with
                // neither is treated as no-auth (e.g. a local endpoint)
                // rather than rejected, so `api_key` presence is not the
                // sole availability predicate.
                Some(p) => {
