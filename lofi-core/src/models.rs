...
//! plexus proxy), and whose `thinking_levels` are inherited from the block.
...
impl ModelRegistry {
    ...
    /// discovered entry that duplicates a static one is dropped. A successful
...
/// `models` map, skipping any id already present so static entries win. New
/// ids are appended in discovery order.
fn inject_discovered(
    models: &mut IndexMap<String, ModelConfig>,
    entries: &[(String, ModelConfig)],
) {
    for (id, mc) in entries {
        models.entry(id.clone()).or_insert_with(|| mc.clone());
    }
}
...
/// config (lofi has no `pi-ai` hints); `reasoning` is inferred as true when
/// any thinking levels are declared.
/// Read a JSON value as `f64`, accepting either a number or a numeric
/// string (Plexus returns pricing as strings like `"5e-7"`).
...
fn parse_auto_models(
    ...
    for entry in arr {
        ...
        // `preferred_api` may be a string or an array (Plexus returns
        ...
        let display_name = entry
            ...
            .map(str::to_string);
        // Plexus (OpenRouter-style) exposes `context_length`, per-token
...
mod tests {
    ...
    #[test]
    fn inject_discovered_skips_static_collision() {
        let mut models = models(&["gpt-4o"]);
        let entries = vec![("gpt-4o".to_string(), mc_named("gpt-4o", "Discovered").1)];
        inject_discovered(&mut models, &entries);
        // Static entry wins: the discovered name is not applied.
        assert_eq!(models.get("gpt-4o").unwrap().name.as_deref(), None);
    }
