//! Retry classification and backoff for provider errors, mirroring Pi's
//! `pi-ai/utils/retry.ts` and `agent-session` auto-retry logic.
//!
...
/// Default cap on retry attempts (not counting the initial try), matching
/// Pi's `settings.retry.maxRetries`.
pub const DEFAULT_MAX_RETRIES: u32 = 3;
/// Base delay for the first retry; subsequent retries double it. Matches Pi's
