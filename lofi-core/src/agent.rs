            ...
            while let Some(ev) = stream.next().await {
                match ev {
                    Ok(e) => {
                        match &e {
                            StreamingEvent::TextDelta(d) => {
                                if !emit(tx, AgentEvent::Text(d.clone())).await {
                                    return Err(Error::Provider("consumer closed".into()));
                                }
                            }
                        ...
                        round_bytes += match &e {
                            StreamingEvent::TextDelta(s) | StreamingEvent::ThinkingDelta(s) => {
                                s.len()
                            }
                            StreamingEvent::ToolUseInputDelta { delta, .. } => delta.len(),
                            _ => 0,
                        };
