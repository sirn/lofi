...
use lofi_code::{exec, AgentFn, ExecCtx, ExecOptions};
...
impl Agent {
    ...
    pub async fn run(&self, user_prompt: String, tx: Sender<AgentEvent>) -> Result<()> {
        ...
        loop {
            ...
            let finished = match self.run_once_inner(&mut messages, Some(&tx)).await {
                Ok(f) => f,
    ...
    pub async fn run_continuation(
        ...
        loop {
            ...
            let finished = match self.run_once_inner(&mut *messages, Some(&tx)).await {
                Ok(f) => f,
                // A gone receiver is a graceful cancellation, not a provider
                // error: stop the run cleanly and roll back any partial turn.
                Err(Error::Cancelled) => {
                    messages.truncate(checkpoint);
                    return Ok(());
                }
    ...
    async fn run_once_inner(
        ...
        if tool_uses.is_empty() {
            // A terminal round always emits exactly one `Done`, defaulting
            // usage when the provider stream omitted a usage chunk (e.g. an
            // OpenAI-compatible endpoint that signals completion via `[DONE]`).
            if let Some(tx) = tx {
                emit(Some(tx), AgentEvent::Done(round_usage.unwrap_or_default())).await;
            }
            return Ok(true);
        }

        let results = self.execute_tools(&tool_uses, tx).await?;
...
/// the CLI overrides.
///
/// Shared by the `lofi-ui` presentation drivers (`run_print`,
/// `run_interactive`) so the resolution ladder (config load → provider
/// selection → `--api-key` override → registry + discovery → model resolution
/// → transport construction) stays in one place. Remote discovery failures
/// fall back to static-only [`ModelRegistry::load`], matching the v1 "do not
/// fatal on discovery" policy.
///
/// # Errors
...
///
/// # Errors
/// Returns [`Error::Config`] if `--provider` names an unknown provider, if
/// `default_provider` refers to a missing provider, or if no provider can be
/// selected (multiple providers, no default).
pub fn select_provider(config: &lofi_types::Config, requested: Option<&str>) -> Result<String> {
    ...
    Err(Error::Config(
        "no provider selected: pass --provider, set default_provider in config, or set OPENAI_API_KEY"
            .into(),
    ))
}
...
///
/// Resolution ladder:
/// 1. if `model_query` is given, resolve within the selected provider and
///    require the result to be available;
/// 2. else use the config's `default_model` (resolved the same way);
/// 3. else the first available model for the selected provider.
///
/// Only models from [`ModelRegistry::available`] (provider is authenticated
/// or explicitly `no_auth`) are eligible, so a configured-but-keyless
/// provider is never silently selected.
///
/// # Errors
...
pub fn select_model(
    ...
    if available.is_empty() {
        return Err(Error::Config(
            "no models available: set OPENAI_API_KEY, pass --api-key, or configure a provider in ~/.config/lofi/config.toml"
                .into(),
        ));
    }
    ...
    if let Some(q) = query {
        // Resolve only within the selected provider. A model from another
        // provider would be encoded with this provider's API and run against
        // its base URL + credentials, cross-wiring the request and leaking
        // credentials to the wrong endpoint.
        if let Some(m) = registry.resolve_for_provider(q, provider) {
            // Require the resolved model to be available (its provider is
            // authenticated or `no_auth`); otherwise a keyless provider's
            // model could be selected and then fail at request time.
            if available
                .iter()
                .any(|a| a.id == m.id && a.provider == m.provider)
            {
                return Ok(m.clone());
            }
            return Err(Error::Config(format!(
                "model `{q}` belongs to provider `{provider}` which is not available; set OPENAI_API_KEY, pass --api-key, or set no_auth = true"
            )));
        }
        return Err(Error::Config(format!(
            "could not resolve model `{q}` for provider `{provider}`; available: {}",
            registry.list_models_print()
        )));
    }

    // No explicit query: the selected provider must have at least one
    // available model. Falling back to another provider's model would
    // encode it with this provider's API and run it against this provider's
    // base URL + credentials — the same cross-wiring the qualified path
    // guards against.
    if let Some(m) = available.iter().find(|m| m.provider == provider) {
        return Ok(m.clone());
    }
    Err(Error::Config(format!(
        "no available model for provider `{provider}`; set OPENAI_API_KEY, pass --api-key, or configure a model for this provider"
    )))
}
...
mod tests {
    ...
    #[test]
    fn select_model_rejects_keyless_provider_model_even_when_other_available() {
        // A second authenticated provider makes `available` non-empty, but an
        // explicit query for a model on the selected keyless provider must
        // still be rejected rather than returned.
        let mut providers = std::collections::HashMap::new();
        providers.insert(
            "openai".to_string(),
            provider_with_key(Api::OpenAiCompletions, vec![mc("gpt-4o")]),
        );
        let mut p = provider_with_key(Api::OpenAiCompletions, vec![mc("local-model")]);
        p.api_key = None;
        providers.insert("local".to_string(), p);
        let reg = registry_with(providers);
        let err = select_model(&reg, "local", None, Some("local-model")).unwrap_err();
        assert!(matches!(err, Error::Config(_)));
    }
    ...
    #[test]
    fn select_provider_uses_default_when_set() {
        let mut providers = std::collections::HashMap::new();
        providers.insert(
            "openai".to_string(),
            provider_with_key(Api::OpenAiCompletions, vec![mc("gpt-4o")]),
    ...
    #[test]
    fn select_provider_errors_when_multiple_and_no_default() {
        let mut providers = std::collections::HashMap::new();
        providers.insert(
            "openai".to_string(),
            provider_with_key(Api::OpenAiCompletions, vec![mc("gpt-4o")]),
        ...
        providers.insert(
            "other".to_string(),
            provider_with_key(Api::OpenAiCompletions, vec![mc("m2")]),
        );
        let cfg = lofi_types::Config {
            providers,
            default_provider: None,
