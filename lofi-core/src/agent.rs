...
use lofi_types::{
    ContentBlock, Message, Model, NativeToolRecord, Role, SessionEvent, StreamingEvent, ThinkingLevel,
    Usage,
};
...
pub enum AgentEvent {
    ...
    /// separately from [`Text`](Self::Text) so the UI can fold it (Pi/Crush
    /// style) while still streaming it live.
    Thinking(String),
    ...
    /// A turn completed: wall-clock duration since the turn started, the
    /// accumulated USD cost across the turn's rounds, and the final round's
    /// token usage. Emitted once per turn by [`Agent::run_continuation`] and
    /// also written to the transcript, so the live view and a resumed view
    /// render the same `◇ label done in Ns` block.
    TurnEnd {
        elapsed_ms: u64,
        cost: f64,
        usage: Usage,
...
impl Agent {
    ...
    /// timing/cost [`SessionEvent`]s to the transcript — making the engine the
    ...
    pub async fn run_continuation(
        ...
        if let Some(commit) = commit {
            let mut events: Vec<SessionEvent> = messages[prev_len..]
                .iter()
                .cloned()
                .map(SessionEvent::Message)
                .collect();
            for (id, dur) in &stats.tool_elapsed {
                events.push(SessionEvent::ToolTiming {
                    id: id.clone(),
                    elapsed_ms: dur.as_millis() as u64,
                });
            }
            for rec in &stats.native_tools {
            ...
            match store::append_events(&commit.path, &events) {
                Ok((byte_start, byte_end)) if byte_end > byte_start && !tx.is_closed() => {
                    let _ = tx
            ...
            let on_tool_event: Arc<dyn Fn(ToolEvent) + Send + Sync> = {
                ...
                Arc::new(move |ev: ToolEvent| match ev {
                    ToolEvent::Start { id, name, args } => {
                        ...
                        if let Some(tx) = &native_tx {
                            let _ = tx.try_send(AgentEvent::NativeToolStart {
                                parent: parent.clone(),
                                id,
                                name,
                                args,
                    ...
                    ToolEvent::End {
                        ...
                        if let Some(tx) = &native_tx {
                            let _ = tx.try_send(AgentEvent::NativeToolEnd {
                                parent: parent.clone(),
                                id,
                                result,
                                is_error,
...
use indexmap::IndexMap;
    use lofi_types::{AgentConfig, Config, ModelConfig, ProviderConfig, ThinkingLevel};
