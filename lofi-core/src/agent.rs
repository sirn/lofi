...
/// sub-tool call would monopolize the exec result. Mirrors Pi's 50 KB /
/// 2000-line per-tool cap.
...
impl TurnStats {
    ...
    fn add_usage(&mut self, usage: Usage, model: &Model) {
        ...
        if let (Some(ip), Some(op)) = (model.input_price, model.output_price) {
            ...
            // parsers); bill the full prompt at the input rate since lofi has
            // no separate cache-read price.
            let prompt = usage.input_tokens + usage.cache_read_tokens + usage.cache_write_tokens;
            self.cost += prompt as f64 / 1_000_000.0 * ip
        ...
        loop {
                ...
                .await
            {
                Ok(true) => {
                    ...
                    // retry, mirroring Pi's `auto_retry_end { success: true }`.
                    if retry_attempt > 0 {
                        let _ = tx
                ...
                Err(e) => {
                    ...
                    // backoff, mirroring Pi's auto-retry: the failed
