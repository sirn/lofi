...
async fn run_shell(cmd: &str) -> Result<String> {
    ...
    let mut guard = crate::tools::builtins::PgrpKillGuard::new(child.id());
...
pub async fn load_config_or_default(path: &Path) -> Result<Config> {
    ...
    match std::fs::symlink_metadata(path) {
        Ok(_) => return load_config(path).await,
...
mod tests {
    ...
    fn lock_env(key: &'static str) -> EnvGuard {
        EnvGuard {
    ...
    async fn resolve_config_lenient_missing_env_leaves_keyless() {
        let _env = lock_env("OPENAI_API_KEY");
        let mut cfg = default_config();
    ...
    async fn load_config_or_default_uses_defaults_and_picks_keyed() {
        // Serialize env-mutating tests and restore the prior value, so this
        // never leaks into the caller environment or races with other tests.
        let _env = lock_env("OPENAI_API_KEY");
