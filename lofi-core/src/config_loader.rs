...
use lofi_types::{Api, Config, DiscoveryConfig, ModelConfig, ProviderConfig};
...
/// A sane built-in configuration used when no user config file exists.
///
/// This lets lofi run out of the box: drop a `$OPENAI_API_KEY` or
/// `$ANTHROPIC_API_KEY` into the environment and the agent works without a
/// `config.toml`. Both providers declare a remote `discover` endpoint (so
/// `--list-models` fetches the live catalog when a key is present) and a
/// single static fallback model (so selection still works offline / when
/// discovery fails). Every `api_key` is an env expression resolved lazily
/// by [`resolve_config_lenient`]; an unset variable simply leaves that
/// provider keyless rather than failing.
#[must_use]
...
/// error, since the user authored it). A missing file yields the built-in
/// defaults resolved *leniently*, and [`Config::default_provider`] is set to
/// the first provider (preferring `openai`, then `anthropic`) whose key
/// resolved — so setting just `OPENAI_API_KEY` is enough to run.
///
/// # Errors
...
pub async fn load_config_or_default(path: &Path) -> Result<Config> {
    ...
    if cfg.default_provider.is_none() {
        let keyed = |name: &str| {
            cfg.providers
                .get(name)
                .is_some_and(|p| p.api_key.as_deref().is_some_and(|k| !k.is_empty()))
        };
        let pick = ["openai", "anthropic"]
            .iter()
            .copied()
            .find_map(|n| keyed(n).then_some(n.to_string()))
            .or_else(|| {
                cfg.providers
                    .iter()
...
mod tests {
    ...
    async fn load_config_or_default_uses_defaults_and_picks_keyed() {
        ...
        // No keys set: both providers keyless, no default chosen.
        std::env::remove_var("OPENAI_API_KEY");
        std::env::remove_var("ANTHROPIC_API_KEY");
        ...
        // Only OPENAI_API_KEY set: openai is keyed and chosen as default.
        ...
        // Only ANTHROPIC_API_KEY set: anthropic is keyed and chosen.
