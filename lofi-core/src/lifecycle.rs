...
use lofi_types::{CompactionConfig, ContentBlock, Message, Role, SessionEvent, SessionEventKind, Usage};

use crate::compact::{compact, compacted_history, CompactOptions, Compaction};
use crate::recall::{recall, recall_cursor, CompactionTarget, RecallOutcome, RecallRequest, RecallScope};
...
impl AgentLifecycle {
    ...
    pub fn history_len(&self) -> usize {
        self.history.lock().map_or(0, |history| history.len())
    }

    #[must_use]
    pub fn history_capacity_bytes(&self) -> usize {
        self.history.lock().map_or(0, |messages| {
            messages.capacity() * std::mem::size_of::<Message>()
                + messages
                    .iter()
                    .map(|message| {
                        message.blocks.capacity() * std::mem::size_of::<ContentBlock>()
                            + message
                                ...
                                .map(|block| match block {
                                    ...
                                    | ContentBlock::Thinking { text, .. } => text.capacity(),
                                    ContentBlock::ToolUse { id, name, input } => {
                                        id.capacity() + name.capacity() + input.to_string().capacity()
                                    }
    ...
    fn compaction_events(&self, cursor: Option<&SessionCursor>) -> Result<Option<Vec<SessionEvent>>> {
        if let Some(cursor) = cursor {
