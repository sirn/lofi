...
/// One-line instruction for the cleared stubs left in the kept tail.
/// Stubs themselves carry only the event id.
pub const RESULT_RECALL_INSTRUCTION: &str = "Cleared tool results and exec source show only an event id. Recover the original with lofi.result(eventId).";
...
mod tests {
    ...
    fn merge_previous_accumulates_goals() {
        ...
        assert!(merged.contains(RESULT_RECALL_INSTRUCTION));
        assert_eq!(
            merged.matches(RESULT_RECALL_INSTRUCTION).count(),
            1,
            "recall instruction must appear once after merge"
        );
    }
