    ...
    for &i in &path[live_start..] {
        match &events[i].kind {
            ...
            SessionEventKind::UserBash {
                command, output, exit_code, signal, duration_ms, truncated,
                cancelled, exclude_from_context,
            } if !skipping && !exclude_from_context => {
                let result = crate::UserBashResult::from_session(
                    command.clone(), output.clone(), *exit_code, *signal,
                    *duration_ms, *truncated, *cancelled,
                );
                live.push(LiveMessage {
                    event_id: events[i].id.clone(),
                    message: Message {
