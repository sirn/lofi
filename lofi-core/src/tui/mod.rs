...
impl App {
    ...
    /// The scroll offset to hand to `Paragraph::scroll`.
    ///
    /// With `log_scroll == 0` we pin to the bottom: skip just enough lines
    /// (total minus the viewport height) so the last page is visible. When the
    /// content is shorter than the viewport nothing is skipped — this is the
    /// case that previously scrolled the whole log out of view. `log_scroll`
    /// scrolls *back up* from that bottom position.
    fn scroll_offset(&self, viewport_height: usize) -> u16 {
        let total = self.total_log_lines();
        let base = total.saturating_sub(viewport_height);
        let off = base.saturating_sub(self.log_scroll);
...
mod tests {
    ...
    fn scroll_clamps_to_log_lines() {
        ...
        // At the bottom with a tall viewport, nothing is skipped.
        assert_eq!(a.scroll_offset(max + 5), 0);
    }
    ...
    fn scroll_offset_pins_bottom_at_zero() {
        ...
        // Taller than the log: the whole log is visible (offset 0).
        assert_eq!(a.scroll_offset(total + 5), 0);
        // Shorter than the log: skip the excess so the last page shows.
        assert_eq!(a.scroll_offset(2), (total - 2) as u16);
        // Scrolling back up reduces the skip by one line.
        a.scroll_up();
        assert_eq!(a.scroll_offset(2), (total - 2 - 1) as u16);
    }
