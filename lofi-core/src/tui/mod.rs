    ...
    fn apply_event(&mut self, ev: AgentEvent) {
        match ev {
            ...
            AgentEvent::ToolStart { id: _, name } => {
                self.messages
                    .push(RenderedMessage::new(Role::Tool, format!("▶ {name}")));
            }
            AgentEvent::ToolInput { id: _, code } => {
                ...
                if !appended {
                    self.messages.push(RenderedMessage::new(Role::Tool, code));
                }
            }
    ...
    match k.code {
        KeyCode::Enter if current_run.is_none() && !app.input.is_empty() => {
            ...
            let handle = tokio::task::spawn_local(async move {
                if let Err(e) = agent_clone.run(prompt, tx).await {
