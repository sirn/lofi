...
/// Mapper from a parsed SSE event to zero or one [`StreamingEvent`].
///
...
/// `(event, data)` pair. Returning `None` skips keep-alive and no-op blocks.
...
where
    F: FnMut(SseEvent) -> Option<StreamingEvent> + Send + 'static,
{
    fn map(&mut self, event: SseEvent) -> Option<StreamingEvent> {
        (self)(event)
    }
...
struct SseState<M> {
    ...
    queued: std::collections::VecDeque<StreamingEvent>,
...
fn append_text<M: SseMapper>(state: &mut SseState<M>, text: &str) {
    // SSE lines are LF-terminated; normalize CRLF so the `\n\n` block
    // delimiter matches regardless of the server's line ending.
...
fn enqueue_block<M: SseMapper>(state: &mut SseState<M>, block: &str) {
    for ev in parse_sse_lines(block.lines()) {
        ...
        if let Some(mapped) = state.mapper.map(ev) {
            state.queued.push_back(mapped);
        }
    }
}
...
mod tests {
    ...
    fn text_mapper(ev: SseEvent) -> Option<StreamingEvent> {
        let v: serde_json::Value = serde_json::from_str(&ev.data).ok()?;
        v.get("text")
            .and_then(serde_json::Value::as_str)
            .map(|s| StreamingEvent::TextDelta(s.to_string()))
    }
