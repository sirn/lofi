...
pub async fn run(
    ...
    // No iteration cap: rely on the model's termination and the per-call
    // timeout enforced inside `round_trip`.
    loop {
        let finished = round_trip.round_trip(&mut messages).await?;
        if finished {
