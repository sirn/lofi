...
impl BuiltinTools {
    ...
    pub async fn find(&self, glob: &str, dir: Option<&str>) -> Result<Value> {
        ...
        let hits = tokio::task::spawn_blocking(move || -> Result<Vec<String>> {
            ...
            // Stop the traversal once we have enough candidates to fill the
            // result cap, instead of materializing the whole tree.
            walk_files_capped(&base, &mut files, MAX_FIND_RESULTS)?;
...
fn walk_files(dir: &Path, out: &mut Vec<PathBuf>) -> Result<()> {
    walk_files_capped(dir, out, usize::MAX)
}

/// Like [`walk_files`] but stops once `out` reaches `cap` entries, bounding
/// memory for callers that only need a bounded result set.
fn walk_files_capped(dir: &Path, out: &mut Vec<PathBuf>, cap: usize) -> Result<()> {
    ...
    for entry in std::fs::read_dir(dir)? {
        ...
        if path.is_dir() {
            walk_files_capped(&path, out, cap)?;
            if out.len() >= cap {
                return Ok(());
            }
        } else {
