...
impl BuiltinTools {
    ...
    pub async fn bash(&self, args: Value) -> Result<Value> {
        ...
        // below: secret-like env vars are scrubbed from the child, and
        // `kill_on_drop` reaps the process tree on timeout or cancellation.
        ...
        match result {
            ...
            Err(_) => {
                // Timeout: `kill_on_drop` kills on drop; kill + reap explicitly
                // so we don't leave a zombie between the timeout and `Drop`.
