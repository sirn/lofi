...
fn summarize_native_args(name: &str, args_json: &str) -> String {
    ...
    let s = match name {
        "read" | "read_tmp" | "edit" | "write" | "ls" => pick(&["path", "file", "dir"]),
        "find" => pick(&["glob", "dir"]),
...
fn extract_path(name: &str, args_json: &str) -> Option<String> {
    ...
    let keys: &[&str] = match name {
        "read" | "read_tmp" | "edit" | "write" | "ls" => &["path", "file", "dir"],
        "find" => &["dir", "glob"],
        "grep" => &["path"],
        _ => return None,
    };
...
fn format_search_output(hits: &[SearchHit], query: &str, header: &str) -> String {
    ...
    for hit in hits {
        ...
        lines.push(format!("{mark} #{} [{}]{} {}", hit.entry.index, hit.entry.role, file_suffix, body));
    }
    lines.join("\n")
}
