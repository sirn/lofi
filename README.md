...
| Diagnostics and resource lifecycle | **Partial** | Debug toggles, transcript result release, and large tree-picker memory release are exercised. | Add debug counters, long-stream memory bounds, repeated session and model switches, dropped consumers, temporary log cleanup, graceful live-run shutdown, and leak checks outside Linux/glibc. |
