...
on a global `pi` object. The agent writes code to use tools, rather than
calling discrete tools one at a time.

v1 ships:

- **Code mode** with a TypeScript → JavaScript (swc strip) → QuickJS runtime.
- Built-in sandbox bindings: `pi.read`, `pi.write`, `pi.edit`, `pi.bash`,
  `pi.ls`, `pi.find`, `pi.grep`.
- **Subagents** via `pi.agent(prompt, opts?)` — a nested agent loop with no
...
```jsonc
{
  ...
  "strings": { "key": "value" }, // optional; exposed as `π` / `pi_strings`
...
- `pi.read(path)` → `string`
- `pi.ls(dir)` → `string` (newline-joined)
- `pi.find(glob, dir?)` → `string`
- `pi.grep(pattern | { regex, ic, ctx, max }, path?)` → `string`
- `pi.write({ path, text })` → `{ ok: true }`
- `pi.edit({ path, old, new })` → `{ ok: true }` (errors if `old` is missing
  or appears more than once)
- `pi.bash({ cmd, timeoutMs? })` → `{ ok, output, code }`
- `pi.agent(prompt, opts?)` → the subagent's final assistant text
