...
| Native file, search, docs, and skill APIs | **Full** | `read`, `ls`, `find`, `grep`, `write`, `edit`, `patch`, `bash`, `skills`, `skill`, `docs`, `docsSearch`, and `tmp_dir`; successful round trips, contained errors, filtering, paging, and truncation recovery. | Filesystem symlink and traversal permutations remain lower-level tests and are tracked separately below. |
